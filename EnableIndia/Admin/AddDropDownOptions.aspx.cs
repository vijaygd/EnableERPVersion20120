...
{
    public partial class AddDropDownOptions : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("~/Admin/AddDropDownOptions.aspx");
        }
        ...
        private void CheckForDuplication()
        {
            ...
            if (duplicateRecords.Equals("1"))
            {
                ...
                Response.Redirect("~/Admin/AddDropDownOptions.aspx?msg=" + message + "&foc=" + Global.EncryptQueryString(BtnAddParameter.ID), true);
            }
        }

        private void CheckForDuplicationForDisabililtySubType()
        {
            string query = "select count(" + DdlParameters.Items[DdlParameters.SelectedIndex].Attributes["IdentityColumn"] + ") ";
            query += "from " + DdlParameters.Items[DdlParameters.SelectedIndex].Attributes["TableName"] + " where " + DdlParameters.Items[DdlParameters.SelectedIndex].Attributes["ColumnName"] + "=\"" + TxtNewType.Text.Trim() + "\"";
            query += " and disability_id=" + DdlDisabilityTypes.Value;
            DBAccess dba = new DBAccess();
            string duplicateRecords = dba.ExecuteQuery(query, null, "Scalar").ToString();
            if (duplicateRecords.Equals("1"))
            {
        ...
        protected void BtnAddParameter_Click(object sender, EventArgs e)
        {
            ...
            if (DdlParameters.Value.ToString().Equals("Disability Sub Type"))
            {
                CheckForDuplicationForDisabililtySubType();
            }
            else
            {
                CheckForDuplication();
        ...
        {
            EnableIndia.App_Code.BAL.DisabilitySubTypesBAL disSub = new EnableIndia.App_Code.BAL.DisabilitySubTypesBAL();
        ...
        private void AddIndustrySegments()
        {
            EnableIndia.App_Code.BAL.CompaniesBAL company = new EnableIndia.App_Code.BAL.CompaniesBAL();
            bool isIndustryAdded = company.AddIndustrySegments(TxtNewType.Text.Trim());
            if (isIndustryAdded.Equals(true))
            {
                Global.ShowMessagesInDiv(this.Page, "Industry Segment added successfully.");
            }
            else
            {
                Global.ShowMessagesInDiv(this.Page, "Error occurred. Please contact the administrator.");
        ...
        {
            EnableIndia.App_Code.BAL.JobsBAL job = new EnableIndia.App_Code.BAL.JobsBAL();
            int isJobAdded = job.AddJob(TxtNewType.Text.Trim());
            ...
            bool isJobRoleAdded = jobRole.AddJobRole(isJobAdded.ToString(), TxtJobRole.Text.Trim());
